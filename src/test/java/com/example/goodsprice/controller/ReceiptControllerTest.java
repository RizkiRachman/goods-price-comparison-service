...
class ReceiptControllerTest {
    ...
    @Test
    @DisplayName("Should allow retry for FAILED receipt")
    void shouldAllowRetry_ForFailedReceipt() {
        // Given - create a FAILED receipt
        byte[] imageContent = "retry-test-content".getBytes();
        String imageHash = calculateSimpleHash(imageContent);

        UUID receiptId = UUID.randomUUID();
        Receipt failedReceipt = new Receipt(receiptId, imageHash, "failed-receipt.jpg");
        failedReceipt.markAsFailed("Simulated failure");
        receiptRepository.save(failedReceipt);

        // When - upload same image again
        MultipartFile image = new MockMultipartFile(
            "image", "retry-receipt.jpg", "image/jpeg", imageContent
        );
        ResponseEntity<ReceiptUploadResponse> response = controller.uploadReceipt(image);

        // Then - should accept the retry (not return as duplicate)
        assertNotNull(response);
        assertEquals(HttpStatus.ACCEPTED, response.getStatusCode()); // New processing, not OK (duplicate)

        ReceiptUploadResponse body = response.getBody();
        assertNotNull(body);
        // Should get a NEW receipt ID (old one deleted)
        assertNotEquals(receiptId, body.getJobId());
    }

    private String calculateSimpleHash(byte[] content) {
